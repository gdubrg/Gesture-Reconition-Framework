...
/// <param name="pImage">pointer in which to return the OpenCV image matrix</param>
/// <returns>S_OK if successful, an error code otherwise</returns>
HRESULT OpenCVFrameHelper::GetDepthDataAsArgb(Mat* pImage, Mat* playerIdxMat) const
{
    ...
    for (UINT y = 0; y < depthHeight; ++y)
    {
		...
		Mat& playerIdxRow = playerIdxMat->row(y);
        ...
        for (UINT x = 0; x < depthWidth; ++x)
        {
            ...
            if (raw_depth != 65535)
            {
				...
				char playerPixel;
                DepthShortToRgb(raw_depth, &redPixel, &greenPixel, &bluePixel,&playerPixel);
                pDepthRgbRow[x] = Vec4b(redPixel, greenPixel, bluePixel, 1);
				playerIdxRow.at<uchar>(x)=playerPixel;
            }
            else
            {
                pDepthRgbRow[x] = 0;
