...
namespace Microsoft {
    namespace KinectBridge {
        ...
        public:
            ...
            /// <returns>S_OK if successful, an error code otherwise</returns>
			HRESULT GetDepthImageAsArgb(Image* pDepthArgbImage, Image* playerIdxMat) const;
        ...
        protected:
            ...
            /// <param name="pImage">pointer in which to return the image data</param>
            /// <returns>S_OK if successful, an error code otherwise</returns>
			virtual HRESULT GetDepthDataAsArgb(Image* pImage, Image* playerIdxMat) const = 0;
            ...
            /// <returns>S_OK if successful, an error code otherwise</returns>
			HRESULT DepthShortToRgb(USHORT depth, UINT8* pRedPixel, UINT8* pGreenPixel, UINT8* pBluePixel, char* playerPixel) const;
        ...
        HRESULT KinectHelper<Image>::Initialize(INuiSensor* pNuiSensor)
        {
            ...
            if (m_hNextColorFrameEvent == 0)
                cout <<"Errore event color"<<endl;
            if (m_hNextDepthFrameEvent == 0)
                cout <<"Errore event Depth"<<endl;
            if (m_hNextSkeletonFrameEvent == 0)
                cout <<"Errore event skel"<<endl;

            cout << "color stream openings" << endl;
            ...
            cout << "depth stream openings" << endl;
            ...
            cout << "skeleton stream opening" << endl;
        ...
        /// <returns>S_OK if successful, an error code otherwise</returns>
        template <typename Image>
        HRESULT KinectHelper<Image>::GetDepthImageAsArgb(Image* pDepthArgbImage, Image* playerIndexMat) const
        {
            // Fail if Kinect is not initialized
            if (!m_pNuiSensor) 
            {
            ...
            hr = GetDepthDataAsArgb(pDepthArgbImage,playerIndexMat);
        ...
        /// <returns>S_OK if successful, an error code otherwise</returns>
        template <typename Image>
        HRESULT KinectHelper<Image>::DepthShortToRgb(USHORT depth, UINT8* redPixel, UINT8* greenPixel, UINT8* bluePixel, char* playerPixel) const
        {
